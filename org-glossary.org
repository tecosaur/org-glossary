...
To define a glossary entry, simply place a top-level section in the document
titled =Glossary= and define terms using a definition list, like so:
...
Then simply use the terms as you usually would when writing, and everything will
work itself out on export :)
...
substitution (fontification), but it kept to a minimum and ideally will be
entirely generalised in future.
...
respecting the various restrictions such as headings and line number ranges. One
can also specify include paths providing definitions that should be globally
...
For example, to define "late pleistocene wolf" one would use a description list
entry like so:
...
unique keys for each usage. In writing one would use the keys like so,
...
To make working with a large collection of terms easier, one might use
sub-headings, e.g.
...
implicit associations to explicit links, one may run =M-x
